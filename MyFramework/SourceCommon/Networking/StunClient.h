...
public:
    bool m_Started;
    double m_TimeSinceLastBindingRequestSent;
    int m_NumberOfBindingRequestsSent;
    bool m_HaveExternalIPAddr;

    bool m_SomethingWentWrong;

    UDPSocket* m_pUDPSocket; // allocated and destroy outside of this class.

    sockaddr_in m_StunServerAddress;

    unsigned long m_ExternalIPAddr;
    int m_ExternalPort;

    unsigned char m_BindingRequest[500];
    int m_BindingRequestLength;
    unsigned char m_BindingResponse[500];
    int m_BindingResponseLength;
...
//    sprintf_s(msg, 64, "%d.%d.%d.%d:%d", ip[0], ip[1], ip[2], ip[3], ntohs(addr->sin_port));
//    return msg;
//}
//
...
//} STUN;
//
//#define TN(id) {static char buf[16]; sprintf_s(buf, 16, "0x%04x", id); return buf; }
