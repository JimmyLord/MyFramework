...
void MySubmesh::SetupMeshSpecificShaderUniforms(Shader_Base* pShader, MyMatrix* pMatProj, MyMatrix* pMatView, MyMatrix* pMatWorld, MyMatrix* matInverseWorld, Vector3* pCamPos, MyLight** pLightPtrs, int numLights, MyMatrix* shadowLightVP)
{
    ...
    checkGlError( "MySubmesh::SetupMeshSpecificShaderUniforms()" );
}
...
void MySubmesh::SetupAttributes(Shader_Base* pShader)
{
    ...
    if( m_pVertexBuffer )
    {
        ...
        {
            if( pVertexBuffer->m_VertexFormat == VertexFormat_Sprite )
                glVertexAttrib3f( pShader->m_aHandle_Normal, 0, 0, -1 );
            else
                glVertexAttrib3f( pShader->m_aHandle_Normal, 0, 1, 0 );
        }
    }
...
void MyMesh::RegisterSetupCustomUniformCallback(void* pObjectPtr, SetupCustomUniformsCallbackFunc pCallback)
{
