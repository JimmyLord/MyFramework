...
#include "FileChangeDetectorWindows.h"

FileChangeDetectorWindows::FileChangeDetectorWindows(const char* folderToWatch)
: FileChangeDetector( folderToWatch )
{
    m_DirectoryHandle = 0;
    m_pBuffer = nullptr;
    ...
    {
        LOGError( LOGTag, "Error with CreateFile in FileChangeDetectorWindows.\n" );
        m_DirectoryHandle = 0;
    }
    else
    {
...
FileChangeDetectorWindows::~FileChangeDetectorWindows()
{
    delete[] m_pBuffer;
...
void FileChangeDetectorWindows::WatchForNextFileSystemChange()
{
...
// Static.
void CALLBACK FileChangeDetectorWindows::NotificationCompletion(DWORD errorCode, DWORD numBytesTransfered, LPOVERLAPPED pOverlapped)
{
    ...
    FileChangeDetectorWindows* pThis = (FileChangeDetectorWindows*)pOverlapped->hEvent;
...
void FileChangeDetectorWindows::CheckForChanges()
{
